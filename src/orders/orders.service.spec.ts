...
describe('OrdersService', () => {
  ...
  beforeEach(async () => {
    ...
    lastOrderNumberModel = module.get<Model<LastOrderNumber>>(
      getModelToken(LastOrderNumber.name),
    );
  });
  ...
  describe('When create is called', () => {
    describe('when one or more products are not available', () => {
      it('should return unprocessable entity exception', async () => {
        jest
          ...
          .mockResolvedValue([productStub()] as any);
        ...
        try {
          ...
          expect(error).toHaveProperty(
            'message',
            'One or more items in your order are invalid or unavailable.',
          );
        }
      });
    });
    ...
    describe('when one or more product items are not available', () => {
      it('should return unprocessable entity exception', async () => {
        const product1 = {
          ...productStub(),
          qty: 2,
          id: '642eb1b706276e3cc9219257',
        };
        const product2 = {
          ...productStub(),
          qty: 2,
          id: '642eb1b706276e3cc9219258',
        };
        jest
          .spyOn(productsService, 'findByIds')
          .mockResolvedValue([product1, product2] as any);
    ...
    describe('when all the items are available', () => {
      it('should create the order and return it', async () => {
        ...
        const lastOrderNUmberSaveStub = {
          ...lastOrderNumberStub(),
          save: jest.fn(),
        };
        jest
          .spyOn(lastOrderNumberModel, 'findOne')
          .mockResolvedValue(lastOrderNUmberSaveStub);
        jest
          .spyOn(lastOrderNUmberSaveStub, 'save')
          .mockResolvedValue({ ...lastOrderNumberStub(), number: 2 });

        jest
          .spyOn(productsService, 'findByIdAndUpdate')
          .mockResolvedValue(productStub() as any);
