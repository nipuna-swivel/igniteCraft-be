...
export class OrdersService {
  constructor(
    ...
    @InjectModel(LastOrderNumber.name)
    private lastOrderNumberModel: Model<LastOrderNumber>,
  ...
  async create(createOrderDto: CreateOrderDto): Promise<Order> {
    ...
    const itemsMap = createOrderDto.items.reduce(
    ...
    if (!lastOrderNumberDoc) {
      lastOrderNumberDoc = await this.lastOrderNumberModel.create({
        number: 0,
      });
    }
  ...
  async updateStatus(
    ...
    if (updateOrderStatusDto.status === OrderStatus.CANCELLED) {
      ...
      const itemsMap = order.items.reduce((accumulator, currentValue) => {
        return {
          ...accumulator,
          [currentValue.product.toString()]: currentValue.qty,
        };
      }, {});
